...
from ..views import academic_performance
from ..models import *
...
def is_valid(data):
    ...
    if ((request_presence == 'False-1') | (request_presence == 'True')):
        request_presence = True
    elif ((request_presence == 'True-1') | (request_presence == 'False')):
        request_presence = False
...
def change_record(request, member_id):
    ...
    if not data:
        ...
        record = AcademicPerformance.objects.get(id=member_id)
        record.is_appeared=data[1]
        record.lesson_date=data[0]
        record.student_mark=data[2]
        record.student_id_id=data[3]
        record.class_id_id=data[4]
        record.subject_id_id=data[5]
        record.save()
        return error
...
def all_shit(request, operation, member_id):
    ...
    if operation == "out":
        ...
        error = change_record(request, member_id)
    elif operation == "add":
